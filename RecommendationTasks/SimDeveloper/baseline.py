...
class MyDataset(Dataset):
    def __init__(self, samples, node_embedding_obj) -> None:
        ...
        for sample in samples:
            ...
            self.data.append([node_embedding_obj.get(src_idx, [0] * feat_size) + node_embedding_obj.get(pos_idx, [0] * feat_size), 1])
            self.data.append([node_embedding_obj.get(src_idx, [0] * feat_size) + node_embedding_obj.get(neg_idx, [0] * feat_size), 0])
...
if __name__ == "__main__":
    ...
    with open(contributor_topic_embedding_path, "rb") as inf:
        node_embedding = pickle.load(inf)

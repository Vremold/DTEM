...
class MyDataset(Dataset):
    def __init__(self, contributor_idx, c_idxs, contributor_embedding, is_tensor=True) -> None:
        super().__init__()
        self.data = []
        src_embedding = contributor_embedding.get(contributor_idx, [0] * 2383)
        if is_tensor:
            src_embedding = src_embedding.cpu().numpy().tolist()
        for c_idx in c_idxs:
            dst_embedding = contributor_embedding.get(c_idx, [0] * 2383)
            if is_tensor:
...
if __name__ == "__main__":
    ...
    for contributor_idx, search_scope, labels in dataset:
        ...
        d = MyDataset(contributor_idx, search_scope, contributor_embedding, is_tensor=False)

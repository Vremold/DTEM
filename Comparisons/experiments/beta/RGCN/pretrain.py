...
SPECIAL_SUFFIX = "pretrain_cat_with_self_loop_baseline_with_only_metapath"
# wandb.init(
#     project="RGCN_pretrain_lp_ec_rgcn_self_loop_15_15_15"
# )
...
class ParameterNamespace():
    def __init__(self, special_suffix):
        ...
        self.checkpoint_dir = f"Comparisons/experiments/beta/RGCN/checkpoint/{special_suffix}_{now}/"
...
def train(model, lp_scorer, train_dataloader, eval_dataloader, test_dataloader, node_feats, epochs, lr=0.001):
    ...
    for epoch in tqdm(range(epochs)):
        ...
        for i, (_, positive_graph, negative_graph, blocks) in tqdm(enumerate(train_dataloader), total=len(train_dataloader)):
            blocks = [b.to(pn.device) for b in blocks]
            positive_graph = positive_graph.to(pn.device)
            negative_graph = negative_graph.to(pn.device)
            x = model(blocks, node_feats)
...
if __name__ == "__main__":
    ...
    exit(0)
    # hg, node_feats, edge2ids = graph_loader.load_test_graph(device=pn.device)

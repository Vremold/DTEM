...
class DatasetGenerator():
    def __init__(self, contributor_idx_file, repo_idx_file, repo_lang_file) -> None:
        ...
        with open(repo_lang_file, "r", encoding="utf-8") as inf:
            obj = json.load(inf)
            self.repo_lang = {}
            for key in self.repo_idx:
                self.repo_lang[key] = obj[key]
        
        self.lang_repo = {}
        for repo, lang in self.repo_lang.items():
            self.lang_repo.setdefault(lang, set()).add(repo)
        pass

    def generate_usr_join_repo(self, contributions_file):
        repo_contributors = {}
        positve_samples = []
        ...
        for sample in positve_samples:
            curr_repo = sample[0]
            curr_user = sample[1]
            lang = self.repo_lang[repo_name]
            
            while True:
                repo = random.choice(list(self.lang_repo[lang] - set([curr_repo])))
...
if __name__ == "__main__":
    ...
    if os.path.exists(positive_cache_path) and os.path.exists(negative_cache_path):
        ...
        positive_samples, negative_samples = dataset_generator.generate_usr_join_repo("../../GHCrawler/cleaned/repo_contributions.txt")
